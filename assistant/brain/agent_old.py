...
def process_turn(user_text: str, conversation_id: Optional[str]) -> Dict[str, Any]:
    ...
    # === NEW: Generic router for non-PROPERTY categories ===
    try:
        if getattr(intent, 'category', None) and str(intent.category).upper() != 'PROPERTY':
            msg = (
                "We don't have that in stock right now, but give us your contact info and we'll contact you soon with the best deals."
            )
            save_assistant_turn(
                conversation_id,
                user_text,
                ...
                message_context={
                    ...
                    "router": "generic_capture_contact"
                }
            )
            return {
                "message": msg,
