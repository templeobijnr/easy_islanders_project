...
from .enterprise_transactions import create_request_safe, approve_broadcast_safe
from .enterprise_guardrails import run_enterprise_guardrails, assess_enterprise_quality
from assistant.monitoring.metrics import PerformanceTracker, extract_token_usage, record_turn_summary

logger = logging.getLogger(__name__)

def _normalize_intent_result(intent: Any) -> Dict[str, Any]:
    """Convert legacy IntentResult objects to a normalized dict."""
    if intent is None:
        return {}
    ...
    if hasattr(intent, "model_dump"):
        try:
            ...
            pass
    # Fallback: pull expected attributes with getattr
    fields = [
        "intent_type",
        "category",
        "confidence",
    ...
    data: Dict[str, Any] = {}
    for field in fields:
        data[field] = getattr(intent, field, None)
    # Some legacy objects expose `extracted_entities`
    if data.get("attributes") in (None, {}, []):
        data["attributes"] = getattr(intent, "attributes", None) or getattr(intent, "extracted_entities", {}) or {}
    return data
...
def nlu_node(state: EnterpriseAgentState) -> EnterpriseAgentState:
    ...
    normalized_intent = _normalize_intent_result(intent_result)
    
    return {
        **state,
        'intent_result': normalized_intent
    }
...
def intent_routing_node(state: EnterpriseAgentState) -> EnterpriseAgentState:
    ...
    intent = _normalize_intent_result(state.get('intent_result'))
    ...
    elif intent_type == 'status_update':
        routing_decision = 'STATUS_UPDATE'
    elif intent_type == 'photo_request':
        routing_decision = 'PHOTO_REQUEST'
    elif category == 'PROPERTY':
        routing_decision = 'PROPERTY_AGENT'
    elif category in ['VEHICLE', 'GENERAL_PRODUCT']:
...
def synthesis_node(state: EnterpriseAgentState) -> EnterpriseAgentState:
    ...
    raw_route = state.get('routing_decision_normalized') or state.get('routing_decision')
    if isinstance(raw_route, dict):
        route_target = raw_route.get('intent_type') or raw_route.get('primary_domain')
    else:
        route_target = raw_route
    agent_name = state.get('target_agent') or state.get('current_node') or route_target or 'enterprise_agent'
    ...
    try:
        ...
        logger.debug("Failed to record turn summary for enterprise synthesis node", exc_info=True)
...
def capture_lead_node(state: EnterpriseAgentState) -> EnterpriseAgentState:
    ...
    request_payload = RequestPayload(
        ...
        location=(intent.get('attributes') or {}).get('location'),
        budget_amount=(intent.get('attributes') or {}).get('budget'),
        currency=(intent.get('attributes') or {}).get('currency', 'EUR'),
...
def broadcast_node(state: EnterpriseAgentState) -> EnterpriseAgentState:
    ...
    from .enterprise_transactions import broadcast_request_atomic
...
def search_external_web(query: str, search_type: str, language: str) -> List[Dict]:
    ...
    try:
        from .enterprise_tools import ExternalWebSearchTool
...
- Respond in {language} language"""),
        ...
        if context_data.get('retry_count') is not None:
            try:
                ...
                logger.debug("Invalid retry_count in enterprise context data", exc_info=True)
        ...
        with tracker as perf:
            ...
            try:
                ...
                logger.debug("Failed to record token usage for enterprise synthesis node", exc_info=True)
...
def should_require_hitl_approval(intent: Dict[str, Any]) -> bool:
    ...
    if intent.get('category') in ['GENERAL_PRODUCT', 'VEHICLE', 'SERVICE']:
        return True
    
    # Require approval if budget is high
    attributes = intent.get('attributes') or {}
    if attributes.get('budget', 0) > 5000:
        return True
    
    return False

def create_approval_gate(request_id: str, intent: Dict[str, Any], conversation_id: str) -> str:
...
def execute_broadcast(request_id: str, intent: Dict[str, Any], language: str) -> Dict:
