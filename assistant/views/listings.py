...
class CategoriesListView(generics.ListAPIView):
    ...
    queryset = Listing.objects.values('category').distinct()
    serializer_class = None  # We'll return simple data
...
class SubcategoriesListView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Listing.objects.filter(category__slug=category_slug).values('subcategory').distinct()

    def list(self, request, *args, **kwargs):
        category_slug = self.kwargs.get('category_slug')
        subcategories = Listing.objects.filter(
...
class MyListingsView(generics.ListAPIView):
    """List listings created by the current user."""
    serializer_class = ListingSerializer
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        return Listing.objects.filter(created_by=self.request.user).order_by('-created_at')
...
class ImageUploadView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        # This would need implementation for actual image upload
        # For now, just return the listing
        instance = self.get_object()
        serializer = self.get_serializer(instance)
        return Response(serializer.data)


class ListingPublishView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        instance = self.get_object()
        instance.status = 'active' if request.data.get('publish', True) else 'draft'
        instance.save()
        serializer = self.get_serializer(instance)
        return Response(serializer.data)


class ListingDuplicateView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        original_id = request.data.get('listing_id')
        try:
            original = Listing.objects.get(id=original_id, created_by=request.user)
...
# Functions moved from views_core.py
