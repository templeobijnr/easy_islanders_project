...
const LeftRail: React.FC = () => {
  ...
  return (
    <aside className="hidden md:block w-72 p-4 border-r border-slate-200 bg-white/90 backdrop-blur">
      ...
      {/* <div className="space-y-2">
        {JOB_CHIPS.map((c) => (
          <JobChip key={c.id} {...c} active={activeJob === c.id} onClick={() => setActiveJob(c.id)} />
        ))}
      </div> */}
...
// Add default export so AppShell's default import works
