...
const GalleryModal: React.FC<GalleryModalProps> = ({ images, title, onClose }) => {
  ...
  return (
    <Dialog open={true} onOpenChange={onClose}>
      <DialogContent className="max-w-none w-full h-full p-0 bg-black/90" onKeyDown={handleKeyDown}>
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        {/* Main Image */}
        <div
          className="relative max-w-5xl max-h-[80vh] w-full mx-4 flex items-center justify-center h-full"
          ...
          {images.length > 1 && (
            <>
              <button
                ...
                className="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-white/10 hover:bg-white/20 backdrop-blur rounded-full transition z-10"
              ...
              <button
                onClick={handleNext}
                className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-white/10 hover:bg-white/20 backdrop-blur rounded-full transition z-10"
