"""
Vehicles Domain Service - manages cars, scooters, boats for sale or rent
"""
...
class VehiclesDomainService(BaseDomainService):
    """Vehicles domain service - manages cars, scooters, boats for sale or rent"""
    
    domain_slug = 'vehicles'
    
    def get_listings(self, seller_user) -> List[Dict]:
        """Get all vehicle listings for a seller"""
        from listings.models import Listing
        
        listings = Listing.objects.filter(owner=seller_user, category__slug='vehicles')
        return [
            {
                'id': str(l.id),
                'title': l.title,
                ...
                'currency': l.currency,
                'vehicle_type': l.dynamic_fields.get('vehicle_type') if l.dynamic_fields else None,
                'mileage': l.dynamic_fields.get('mileage') if l.dynamic_fields else None,
                'transmission': l.dynamic_fields.get('transmission') if l.dynamic_fields else None,
            }
            for l in listings
        ]
    
    def get_metrics(self, seller_user, period='month') -> Dict[str, Any]:
        """Get vehicle metrics for analytics"""
        from listings.models import Listing
    ...
    def get_bookings(self, seller_user) -> List[Dict]:
        """Get all vehicle bookings/rentals for a seller"""
        from bookings.models import Booking
        ...
        return [
            {
                'id': str(b.id),
                'title': f"Rental: {b.listing.title}",
    ...
    def create_listing(self, seller_user, payload: Dict) -> Dict:
        ...
        category, _ = Category.objects.get_or_create(
            slug='vehicles',
            defaults={'name': 'Vehicles', 'is_bookable': True}
        )
    ...
    def update_listing(self, seller_user, listing_id: str, payload: Dict) -> Dict:
        ...
        listing = Listing.objects.get(id=listing_id, owner=seller_user, category__slug='vehicles')
    ...
    def get_listing_detail(self, listing_id: str) -> Dict:
        """Get detailed vehicle listing info"""
        from listings.models import Listing
        from bookings.models import Booking
        
        listing = Listing.objects.get(id=listing_id, category__slug='vehicles')
        ...
        return {
            ...
            'vehicle_type': listing.dynamic_fields.get('vehicle_type') if listing.dynamic_fields else None,
            'mileage': listing.dynamic_fields.get('mileage') if listing.dynamic_fields else None,
            'transmission': listing.dynamic_fields.get('transmission') if listing.dynamic_fields else None,
            'total_bookings': bookings.count(),
